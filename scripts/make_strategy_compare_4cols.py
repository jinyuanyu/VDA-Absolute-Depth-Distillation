...
def main() -> None:
    ...
    col_titles = ["RGB", "Depth-Pro", "VDA Inverse", "Multi-Scene Model"]
    ...
    for i, scene in enumerate(SCENES):
        ...
        axes[i, 3].imshow(depth_vis(d_multi))

        axes[i, 3].set_xlabel(f"MSE={mse_multi:.4f}", fontsize=11)
    ...
    fig.suptitle("4-Column Comparison (Latest Multi-Scene Weights)", fontsize=18)
    plt.tight_layout(rect=[0, 0.02, 1, 0.95])

    out_png = OUT_DIR / "compare_rgb_depthpro_vdainv_multi_2scene_4cols.png"
    out_json = OUT_DIR / "compare_rgb_depthpro_vdainv_multi_2scene_4cols_metrics.json"
